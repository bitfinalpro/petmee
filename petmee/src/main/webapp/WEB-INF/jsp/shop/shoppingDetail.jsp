...
<link rel="stylesheet" type="text/css"
	href="<c:url value="/resources/css/shopping/detail.css" />">
<link href="<c:url value="/resources/css/shopping/login/login2.css " />"
	rel="stylesheet">
<link href="<c:url value="/resources/css/shopping/login/join.css " />"
	rel="stylesheet">
<script
	src="../resources/js/shopping/login/lib/jquery.magnific-popup.js"></script>
<script src="<c:url value="/resources/js/common/jquery-3.4.1.js" />"></script>
<script
	src="<c:url value="/resources/js/shopping/main/soppingDetail.js" />"></script>
<script
		...
		<form method="post"
			...
			<div class="inner product">
				...
				<div class="product_img">
					<div id="imgGallery">
						<div id="smallImgs">
							<input name="image" value="<c:url value='${imgfirst}'/>"
								type="hidden" />
						...
						<div id="bigImg">
							<c:forEach var="f" begin="0" end="0" items="${flist}"
								varStatus="status">
				...
				<div class="proinfo">
					<h2>
						...
						<input class="allprice" name="price"
							value="${product.price - product.dcPrice}" type="hidden" /> <strong><input
							value=" ${product.productName}" /></strong>
					</h2>
					...
					<div class="pinfo-txt">
						<table>
							...
							<tbody class="pinfo-price">
								<tr>
									<th>판매가</th>
									<td><del id="productPrice">
											<em><input class="priceInput"
												value="<fmt:formatNumber type="number" maxFractionDigits="3" value="${product.price}" />"
												readonly /></em>원
										</del> <span class="txt-price"><em><input
												class="priceInput" name="price1"
												value="<fmt:formatNumber type="number" maxFractionDigits="3" value="${product.price - product.dcPrice}" />"
												readonly /> <fmt:formatNumber type="number"
													maxFractionDigits="3" value="" /></em>원</span></td>
								</tr>
							</tbody>
							...
							<tbody class="pinfo-info">
								<tr>
									<th>수량 :</th>
									<td><input id="amount" name="amount" class="excessCnt"
										type="hidden" value=1 readonly /> <input
										class="excessCnt subinput" name="excessCnt" value=1 readonly /><span>개</span>
										<button class="amount_btn" onclick="addtnNofpr.add('amount')"
											type="button">+</button>
					...
					<fieldset>
						<div class="pro-total">
							<strong>총 상품금액(수량)</strong> <em id="totalPrice_A"> <input
								class="price" name="price" class="inputRltvGoodsAmt"
								value="<fmt:formatNumber type="number" maxFractionDigits="3" value="${product.price - product.dcPrice}" />" />
							</em>원 (<input id="asd" value="1" name="excessCnt" class="excessCnt"
								onchange="change();" />개)
						</div>
						<!-- default// -->
						<div class="btn-area btnarea-default active">
							<c:choose>
								<c:when test="${empty user }">
									<a onclick="$('#login-pop').modal('show');" href="javascript:;"><button
											type="button" class="btn-black btn-buy">바로 구매하기</button></a>
									<button type="submit" class="btn-gray btn-cart">장바구니
										담기</button>
								</c:when>
								<c:otherwise>
								<div id="y-hide"></div>
			...
			<div class="returninfo" id="returnInfo">
				...
				<table>
					...
					<tbody>
						<tr>
							<th>배송정보</th>
							<td>-모나미 물류배송의 경우 :&nbsp;실 결제금액(예치금 포함) 2만원 이상일 경우
								무료배송입니다.&nbsp;무료배송 상품이 포함됐을 경우 해당 상품은 구매금액&nbsp;합산에서
								제외됩니다.&nbsp;<br /> -업체직배송 상품 중 무료배송이 아닌 경우 : 업체개별 배송비 정책에 따라
								배송비가 개별 부과됩니다.&nbsp;<br /> -배송비 정책 제외 상품의 경우(개별배송비 부과) : 배송비
								정책과는 무관하게 상품에 부과되는 배송비로 배송정책 비용에 비포함되며, 수량*배송비가 부과됩니다. 배송비 정책 제외
								대상 상품은 무료배송 조건 합산금액에는 포함되지 않습니다.&nbsp;<br /> -도서산간비 : 2만원 미만 구매
								시 기본 배송료 2,500원에 추가 도선료가 부과됩니다. 2만원 이상 구매 시에는 기본 배송료가 무료이므로 추가
								도선료만 부과됩니다. 도서산간비의 경우, 모나미 물류배송, 업체직배송 모두 기준 공통으로 적용합니다.&nbsp;
							</td>
						</tr>
						<tr>
							<th>주문취소</th>
							<td>-주문 상태가 &lsquo;입금대기&rsquo;, &#39;결제완료&#39; 일 경우는 모나미몰
								마이페이지에서 신청하실 수 있습니다.<br /> -주문 상품의 상태가 &lsquo;배송준비중&rsquo;,
								&lsquo;배송중&rsquo;, &lsquo;배송완료&rsquo;인 경우는 주문 취소 신청이 진행이 되지 않으며,
								반품, 교환으로 진행한 뒤 제품 회수 후 환불 처리됩니다. 환불 처리는 제품 회수 완료 시점으로 최대 15일 이내에
								처리해 드립니다.&nbsp;&nbsp;
							</td>
						</tr>
						<tr>
							<th>교환안내</th>
							<td>교환은 &lsquo;배송중&rsquo;, &#39;배송완료&#39;의 상태일 때 신청이 가능하며
								모나미몰 마이페이지에서 신청하실 수 있습니다. 교환신청 후 교환을 원하는 상품을 먼저 모나미몰, 또는 판매자에게
								발송하고 모나미몰 또는 판매자가 상품 확인 후 교환 상품 발송이 이루어집니다.</td>
						</tr>
						<tr>
							<th>반품안내</th>
						...
						<tr>
							<th>교환/반품 배송비 안내</th>
							<td>결제수단에 따라 처리 기간의 차이가 날 수 있으며, 모나미몰의 경우, 토/일요일, 공휴일 제외
								5~7일 정도 소요됨을 알려드립니다. 판매자 상품의 경우, 모나미몰 소요기간과 다를 수 있음을
								알려드립니다.&nbsp; &nbsp;</td>
						</tr>
						<tr>
							<th>반품/교환 불가사유 안내</th>
							<td>- 제품 하자, 오배송의 이유로 반품, 교환 시에는 모나미몰 또는 판매자가 배송비를 부담합니다. 단,
								...
								왕복 배송비 부과)를 고객님께 부과할 수 있습니다.<br /> - 고객님 책임에 해당하는 사유로 상품 및 구성품
								등이 멸실훼손된 경우(개봉된 제품), 고객님 사용 또는 일부 소비에 의해 상품 가치가 감소한 경우, 상품 제작에
								들어가는 각인 서비스의 경우, 그 외 전자상거래 등에서 소비자보호에 관한 법률이 정하는 청약철회 제한에 해당하는
								경우에는 반품, 교환이 불가함을 알려드립니다.&nbsp;<br /> - 단순변심, 고객님의 개인적인 사정으로 인해
						...
						<tr>
							<th>사은품 안내</th>
							<td>- 사은품은 프모로션 내용에 따라 지급 대상이 다르게 적용될 수 있습니다.<br /> - 사은품은
								소진 시 사전 예고 없이 변경되거나 지급이 종료될 수 있습니다.<br /> - 주문 취소, 교환, 반품 등 주문
								상태 변경 시 사은품이 회수될 수 있습니다.
							</td>
						</tr>
					</tbody>
				</table>
				...
				<table class="notice_tb">
					...
					<c:forEach var="b" items="${list}">
						<tr<%-- onclick="document.location.href='detail.do?no=${b.no}'" --%>>
							...
							<fmt:formatDate pattern="yyyy-MM-dd" value="${b.regDate}" /></td>
							<td>★★★☆☆</td>
							<td>${b.viewCnt}</td>
							<td><a
								href="<c:url value="/board/reviewboard/updateform.do?no=${b.no}"/> "><button>수정</button></a>
								<a
								href="<c:url value="/board/reviewboard/delete.do?no=${b.no}"/> "><button>삭제</button></a>
							</td>
						</tr>
					</c:forEach>
				</table>
						...
						<ul class="pagination">
							<c:forEach var="i" begin="${pr.beginPage}" end="${pr.endPage}">
								<li <c:if test="${pr.pageNo == i}">class="active"</c:if>><a
									href="review.do?pageNo=${i}">${i}</a></li>
							</c:forEach>
						</ul>
					</nav>
				</div>
				...
				<div id="page">
					<nav>
						<ul class="pagination">
							<c:forEach var="i" begin="${pr.beginPage}" end="${pr.endPage}">
								<li <c:if test="${pr.pageNo == i}">class="active"</c:if>><a
									href="notice.do?pageNo=${i}">${i}</a></li>
							</c:forEach>
						</ul>
					</nav>
				</div>
			...
			<div class="bottombuybtn">
				<div class="inner">
					<div class="bbuybtn" id="">
						<span class="txt-total">총 상품금액(수량) <strong><em
								id="totalPrice_B"><input class="price" name="price1"
									class="inputRltvGoodsAmt"
									value="<fmt:formatNumber type="number" maxFractionDigits="3" value="${product.price}" />">
									(<input value="1" name="excessCnt" class="excessCnt"
									onchange="change();" />개)</strong></span>
						<!-- default// -->
						<div class="btn-area btnarea-default active">
